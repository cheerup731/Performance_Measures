...
for (m in 1:M) {
  ...
  tb = matrix(data = NA, nrow = length(senario)+1, ncol = 11) 
  colnames(tb) = c('error', 'cal1', 'cal2', 'cal3', 'msb_00', 'msb_05', 'msb_10', 'entropy', 'brier', 'KS', 'AIC')
  ...
  tb[g,] = c(error, cal1, cal2, cal3, msb_00, msb_05, msb_10, entropy, brier, KS, aic)
  }
  ...
  tb[g+1, c(1:10)] =  c(error, cal1, cal2, cal3, msb_00, msb_05, msb_10, entropy, brier, KS)
  ...
  tau = tau + c(cor(tb[,1], tb[,2], method = "kendall"),
                ...
                cor(tb[,1], tb[,9], method = "kendall"),
                cor(tb[,1], tb[,10], method = "kendall", use = "complete.obs")) /M
  
 }

tau
library("xtable")
xtable(rbind(c('Cal1', 'Cal2', 'Cal3', 'msb00', 'msb05', 'msb10', 'Entropy', 'Brier', 'KS', 'AIC'),round(tau,2)), caption = 'kendall Tau')
